          ...
          <div className="mt-3 text-center flex flex-col items-center justify-center w-full">
            {app.priceOriginal && app.priceDiscounted ? (
              <>
                ...
                <span className="text-[#d95d39] text-sm md:text-base font-semibold group-hover/link:text-orange-700 transition-colors">
                  Learn More
                </span>
              </>
            ) : app.priceDiscounted ? (
              ...
              <span className="mt-2 text-[#d95d39] text-sm md:text-base font-semibold group-hover/link:text-orange-700 transition-colors">
                Learn More
              </span>
            )}
          </div>
...
export function AppCategory({ title, deviceImageAlt, deviceImagePath, imageClassName, apps, reverse = false }: AppCategoryProps) {
  return (
    <section className="py-4 md:py-8 w-full max-w-[1920px] mx-auto px-2 md:px-4">
      {/* Title */}
      <h2 className="text-3xl font-bold mb-4 text-gray-900 ml-4 md:ml-12 hidden">{title}</h2>
      
      <div className={`flex flex-col md:flex-row items-center justify-center gap-8 md:gap-16 px-4 md:px-12 ${reverse ? 'md:flex-row-reverse' : ''}`}>
        {/* Device Image */}
        <motion.div 
        ...
        {/* Apps Grid */}
        <div className="w-full md:w-2/3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 text-center">
          {apps.map((app, index) => (
